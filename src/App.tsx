...
export default function App() {
  ...
  if (!playerSide) {
    return <main className="setup-shell"><section className="setup-card">
      <div className="brand-mark">XC</div><span className="eyebrow">STOCKFISH 18 · LOCAL</span><h1>Xadrez Coach</h1>
      <p>Escolha como quer usar o tabuleiro. Todo o processamento continua local no navegador.</p>
      <div className="side-options">
        <button className="side-option white-option" onClick={() => startWithSide('w')}><span className="side-piece">♔</span><strong>Coach · Brancas</strong><small>Recomendações focadas nas brancas.</small></button>
        <button className="side-option black-option" onClick={() => startWithSide('b')}><span className="side-piece">♚</span><strong>Coach · Pretas</strong><small>Recomendações focadas nas pretas.</small></button>
      </div>
  ...
  return <main className="app-shell">
    ...
    <aside className="coach-panel">
      ...
      <section className={`card recommendation-card ${recommendationActive ? 'active' : ''}`}><div className="card-title"><span className="section-label">MELHOR JOGADA</span>{thinking && <span className="mini-loader" />}</div>{recommendationActive ? <>{bestMove ? <><div className="move-hero"><b>{bestSan}</b><span className="uci-move">{bestMove.slice(0, 2)} → {bestMove.slice(2, 4)}</span></div><p>{explainMove(liveGame, bestMove)}</p><div className="idea-tags">{tacticalIdeas(liveGame, bestMove).map((idea) => <span key={idea}>{idea}</span>)}</div></> : <span className="muted">Calculando…</span>}</> : <div className="waiting-coach"><strong>Primeiro mova o adversário</strong><p>Depois o coach recalcula sua melhor resposta.</p></div>}</section>
