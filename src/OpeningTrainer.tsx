...
export default function OpeningTrainer({ engine, pieceSet }: { engine: StockfishEngine | null; pieceSet: string }) {
  ...
  return <main className="trainer-shell">
    <header className="trainer-topbar"><div><span className="eyebrow">MODO PROFESSOR</span><h1>Estúdio de Aberturas</h1></div></header>
    <div className="trainer-layout">
      <aside className="trainer-sidebar">
        ...
        <div className="trainer-course-list">
          {categories.map((category) => {
            ...
            return <div key={category} className="trainer-category"><span className="trainer-category-label">{category}</span>{items.map((item) => <button key={item.id} className={`trainer-course-item ${item.id === course.id ? 'active' : ''}`} onClick={() => selectCourse(item)}><strong>{item.name}</strong><span>{item.eco}</span></button>)}</div>
          })}
      ...
      <div className="trainer-board-column">
        ...
        <div className="trainer-controls">
          {mode === 'lesson' ? <><button onClick={retreatLesson} disabled={stepIndex === 0}>← Lance anterior</button><span>{stepIndex} / {course.steps.length}</span><button onClick={advanceLesson} disabled={stepIndex >= course.steps.length}>Próximo lance →</button></> : <><button onClick={restart}>Reiniciar linha</button><span>{attempted.size} / {totalQuizMoves} testados · {mistakes.size} erro(s)</span></>}
        </div>
      </div>

      <aside className="trainer-panel">
        <section className="card trainer-course-card"><div className="card-title"><strong>{course.name}</strong><span>{course.eco}</span></div><p>{course.summary}</p><div className="trainer-mode-toggle"><button className={mode === 'lesson' ? 'selected-tool' : ''} onClick={() => switchMode('lesson')}>Lição explicada</button><button className={mode === 'quiz' ? 'selected-tool' : ''} onClick={() => switchMode('quiz')}>Treino ativo</button></div><div className="trainer-side-toggle"><span>Você joga com</span><button className={studentSide === 'w' ? 'selected-tool' : ''} onClick={() => switchSide('w')}>Brancas</button><button className={studentSide === 'b' ? 'selected-tool' : ''} onClick={() => switchSide('b')}>Pretas</button></div></section>
        {mode === 'lesson' && <section className="card trainer-step-card"><div className="card-title"><strong>Explicação</strong><span>lance {stepIndex} de {course.steps.length}</span></div>{previousStep ? <><b>{previousStep.san}</b><p>{previousStep.comment}</p></> : <p className="empty-state">Posição inicial. Clique em "Próximo lance" para começar a lição.</p>}{currentStep && <p className="trainer-next-hint">A seguir: <code>{currentStep.san}</code></p>}{!currentStep && <p className="trainer-end-note">Fim da linha estudada. Você pode continuar explorando a posição no modo Análise livre do tabuleiro principal.</p>}</section>}
        {mode === 'quiz' && <section className={`card trainer-step-card ${feedback?.kind ?? ''}`}><div className="card-title"><strong>Treino ativo</strong>{checking && <span className="mini-loader" />}</div>{finished ? <div><strong>Linha concluída!</strong><p>{mistakes.size === 0 ? 'Você acertou todos os lances de memória. Excelente domínio dessa linha.' : `Você errou ${mistakes.size} lance(s) nesta tentativa. Reinicie para treinar novamente.`}</p></div> : isStudentTurn ? <p>Sua vez: encontre o lance principal da linha para {studentSide === 'w' ? 'as brancas' : 'as pretas'}.</p> : <p className="muted">O oponente está jogando o lance de livro...</p>}{feedback && <div className={`trainer-feedback ${feedback.kind}`}>{feedback.text}</div>}{(feedback?.kind === 'mistake' || feedback?.kind === 'off-book') && !finished && <button className="trainer-reveal" onClick={playCorrectMove} disabled={checking}>Jogar o lance certo e continuar</button>}</section>}
      </aside>
    </div>
  </main>
}
